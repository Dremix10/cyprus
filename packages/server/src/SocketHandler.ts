  ...
  private updateLeaderboardStats(
    ...
    for (const { pos, userId, disconnected } of playersToUpdate) {
      ...
      this.db!.updateUserStats(userId, {
        won,
        firstOut,
        tichuCall: playerTichus.some((r) => r.call === 'tichu'),
        tichuSuccess: playerTichus.some((r) => r.call === 'tichu' && r.success),
        grandTichuCall: playerTichus.some((r) => r.call === 'grand_tichu'),
        grandTichuSuccess: playerTichus.some((r) => r.call === 'grand_tichu' && r.success),
